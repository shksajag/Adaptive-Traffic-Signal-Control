...
I've used the following software versions: Python 3.13, SUMO traffic simulator 1.24.0, tensorflow 2.0
...
**Training time:** ~120 seconds per episode, ~4 hours for 100 episodes.
...
In the "intersection" folder, there is a file called *baneswor_final.net.xml*, which defines the environment's structure, and it was created using SUMO NetEdit. The other file *simubaneswor.sumocfg* it is a linker between the environment file and the route file.  
...
**Environment**: a 4-way intersection model of the Baneshwor intersection. Each incoming lane defines the possible directions that a car can follow: left-most lane dedicated to left-turn only; right-most lane dedicated to right-turn and straight.
